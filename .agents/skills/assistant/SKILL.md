...
AI가 투자 쪽으로 판단하는 곳은 두 군데뿐입니다. 2주차에서는 미국 대형주 후보의
약점과 반대 근거를 읽을 때, 체결이 난 뒤 결과만 읽을 때(리뷰어). 설치·에러 수정은
코딩 도우미예요. 종목을 고르는 판단은 3·4회차입니다."

## 정해진 시각에 자동 실행
학생이 "매주 자동으로 / 자는 동안 / hermes"를 물으면
`lessons/2부-나만의-에이전트/4-자동화-hermes-예약.md` 로 안내한다.
- **본편**: `hermes setup --portal` (Nous 무료) → 스크립트 복사 → 말로 예약.
  판단(인터뷰·리뷰어)은 Claude Code, 자는 동안 시작은 hermes.
