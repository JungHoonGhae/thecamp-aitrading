...
## 🎛️ 내 스펙 바꾸기 — 한 파일만 골라도 됩니다

기본값 그대로 완주해도 되고, 바꾸고 싶으면 **한 파일만** 골라 AI에게 말하세요.

| 바꾸고 싶은 것 | 파일 | 이렇게 말하면 됩니다 |
|---|---|---|
| 담는 종목·비중 | `agent/spec/portfolio.md` | "3종목 균등으로 줄여줘. 바꾼 뒤 `python verify.py` 로 검증하고 전후 차이를 설명해줘." |
| 점검 주기·민감도 | `agent/spec/rules.md` | "허용 오차를 5%p → 3%p 로 좁혀줘. 뭐가 달라지는지 예시로 설명해줘." |
| 안전선 | `agent/spec/guardrails.md` | "더 보수적으로 바꿔줘 — 한 종목 최대 30%, 현금 최소 10%." |

수정 후 검증까지 시키는 게 요령입니다 — "고치고, 검증하고, 전후를 설명해줘."
