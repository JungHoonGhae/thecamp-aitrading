...
|---|---|---|---|
| 1️⃣ 수업 기본 | (아무 설정 없음 — 지금 이 상태) | 아무것도 — 고정 데이터로 항상 동일하게 동작 | 없음 |
| 2️⃣ 모의투자 | `KIS_MODE=live` | 시세·계좌·주문이 진짜 (돈은 가짜) | KIS 모의투자 App Key |
| 3️⃣ 실전 | `KIS_ENV=real` + `KIS_REAL_ACK=REAL-MONEY-OK` | **전부 진짜 — 실제 돈** | 실전 계좌 키 + 이중 확인 |
...
| 단계 | 방법 | 예시 |
|---|---|---|
| 1️⃣ 기계식 | 표의 값만 고치고 `python sync_spec.py` | ③ 오차를 5 → 3 으로 |
| 2️⃣ 응용 | AI에게 의도만 말하기 | "배당 위주 3종목으로 표를 채우고 sync·verify까지 해줘" |
| 3️⃣ 고급 | `agent/spec/` 파일을 직접 다듬기 | guardrails.md 에 나만의 금지 추가 |
