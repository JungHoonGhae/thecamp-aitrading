"""hermes gateway(no-agent)용 점검 스크립트.

미리보기만 한다(주문 없음). 실행판은 portfolio-rebalance.py.
...
2주차 미국 참조 실험은 이 스크립트가 아니다.
정보만: `python routines/참조전략-실험.py --no-send`
채택·연습 계좌: `python agent/committee.py` 와 `python agent/us_agent.py`
